...
std::pair<std::vector<TradeRecord>, Quote> OrderBook::process_order(Quote& quote, bool from_data, bool verbose)
{
...
std::pair<int, std::vector<TradeRecord>> OrderBook::process_order_list(const std::string& side,
...
std::vector<TradeRecord> OrderBook::process_market_order(Quote& quote, bool verbose)
  {
    std::vector<TradeRecord> trades;
...
std::pair<std::vector<TradeRecord>, Quote> OrderBook::process_limit_order(
...
void OrderBook::modify_order(int order_id, const Quote& order_update, int time) {
  if (time != 0) {
    this->time = time;
  } else {
    this->update_time();
...
double OrderBook::get_worst_bid() {
...
double OrderBook::get_worst_ask() {
  return asks.max_price();
}

void OrderBook::tape_dump(const std::string& filename, const std::string& filemode, const std::string& tapemode) {
