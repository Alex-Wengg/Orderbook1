...
struct TradeRecord {
  ...
  std::string toString() const {
    ...
    ss << "timestamp: " << timestamp << " price: " << price
...
public:
  ...
  process_order(Quote &quote, bool from_data, bool verbose);

  std::pair<int, std::vector<TradeRecord>>
  process_order_list(const std::string &side, OrderList &order_list,
                     int quantity_still_to_trade, Quote &quote, bool verbose);

  std::vector<TradeRecord> process_market_order(Quote &quote, bool verbose);

  std::pair<std::vector<TradeRecord>, Quote>
  process_limit_order(Quote &quote, bool from_data, bool verbose);
  ...
  void tape_dump(const std::string &filename, const std::string &filemode,
